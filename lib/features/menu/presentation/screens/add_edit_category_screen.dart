...
class _AddEditCategoryScreenState extends State<AddEditCategoryScreen> {
    ...
    for (var day in _daysOfWeek) {
      if (widget.category != null && widget.category!.horarios.isNotEmpty) {
        ...
        if (existing.startTime != null && existing.startTime!.contains(':')) {
          ...
          _startTimes[day] = TimeOfDay(hour: int.parse(parts[0]), minute: int.parse(parts[1]));
        }
        if (existing.endTime != null && existing.endTime!.contains(':')) {
          final parts = existing.endTime!.split(':');
          _endTimes[day] = TimeOfDay(hour: int.parse(parts[0]), minute: int.parse(parts[1]));
        }
      } else {
  ...
  List<CategorySchedule> _buildSchedulesFromState() {
    ...
    for (var day in _daysOfWeek) {
      schedules.add(CategorySchedule(
        day: day,
        startTime: _startTimes[day] != null ? _formatTime(_startTimes[day]) : null,
        endTime: _endTimes[day] != null ? _formatTime(_endTimes[day]) : null,
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text("Horarios de Disponibilidad", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(isEditing ? 'Actualizar Categoría' : 'Crear Categoría', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                      ),
                    ),
                  ],
                ),
            ...
            Row(
              children: [
                Checkbox(
                  ...
                  onChanged: (val) => setState(() => _activeDays[day] = val!),
                ),
                Text(day, style: TextStyle(fontWeight: FontWeight.w600, color: isActive ? Colors.black87 : Colors.grey)),
              ],
            ),
            if (isActive)
              Padding(
      ...
      child: InkWell(
        onTap: () async {
          final TimeOfDay? picked = await showTimePicker(
            context: context,
            initialTime: (isStart ? _startTimes[day] : _endTimes[day]) ?? const TimeOfDay(hour: 8, minute: 0),
          );
          if (picked != null) {
            setState(() {
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                (isStart ? _startTimes[day] : _endTimes[day]) != null ? _formatTime(isStart ? _startTimes[day] : _endTimes[day]) : (isStart ? 'Inicio' : 'Fin'),
